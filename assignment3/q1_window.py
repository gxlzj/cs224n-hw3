    ...
    ### YOUR CODE HERE (5-20 lines)
        for i in xrange(len(sentence)) :
            ...
            for j in xrange(i - window_size, i + window_size + 1) :
                if (j < 0) :
                    windowFeature.extend(start)
                elif (j >= len(sentence)) :
                    windowFeature.extend(end)
                else :
                    windowFeature.extend(sentence[j])
            windowed_data.append((windowFeature, labels[i]))
    ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.nn.embedding_lookup(embeddings_all, self.input_placeholder)         
